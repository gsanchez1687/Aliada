<template>
  <div class="page-body">
    <CommonBreadcrumb title="Editar propiedad" page="Mis propiedades" />
          ...
          <div class="card">
            ...
            <div class="card-body admin-form">
              <form class="row gx-3" @submit.prevent="saveProperty">
                ...
                <div class="col-sm-4 form-group">
                  <label>Estado</label>
                  <select class="form-control" v-model="property.state_id">
                    <option disabled value="">Seleccionar</option>
                    <option
                ...
                <div class="mt-3 mb-3">
                  <div class="dropzone-1">
                    ...
                    >
                      ...
                      <p v-if="isDragActive">Soltar la imagenes aqui...</p>
                      <p v-else>
                ...
                <!-- Submit Buttons -->
                <div v-if="(Number(property.approval_state_id) === 1 )" class="form-btn col-sm-12">
                  <button type="button"  @click="changeStatus(3)" class="btn btn-pill btn-reject">
                    Rechazar
                  </button>
                  <button type="button" class="btn btn-pill btn-gradient color-4" @click="changeStatus(2)">
                    Aprobar
                  </button>
                </div>
                <div class="top" v-else>
                  <button type="button" class="btn btn-pill btn-gradient color-4" @click="saveProperty()">
                    Actualizar
                  </button>
                </div>
              </form>
...
const property_slug = ref(router.currentRoute.value.params.id);
...
onMounted(async () => {
  await backendServices
    .getPropertyBySlug(property_slug.value)
    .then((response: any) => {
      property_id.value = response.data.id
      property.value.type_property_id = response.data.type_property_id;
      property.value.category_id = response.data.category_id;
      property.value.label_id = response.data.label_id;
      property.value.price = response.data.price;
      property.value.rooms = response.data.rooms;
      property.value.bathroom = response.data.bathroom;
      property.value.square_meters = response.data.square_meters;
      property.value.views = response.data.views_property[0].view_id;
      property.value.description = response.data.description;
      property.value.address = response.data.address;
      property.value.postalCode = response.data.postalCode;
      property.value.state_id = response.data.state_id;
      property.value.municipality_id = response.data.municipality_id;
      property.value.title = response.data.title;
      property.value.amenities = response.data.amenities.map(
        (amenity) => amenity.amenity_id
      );
      property.value.parking = response.data.parking;
      property.value.coordinates_x = response.data.coordinates_x;
      property.value.coordinates_y = response.data.coordinates_y;

      response.data.images.forEach((image) => {
        getImages(image.url).then((file) => {
  ...
  await backendServices
    ...
    await backendServices
    .getStatusProperty(property_id.value)
    .then((response: any) => {
      console.log(response.data)
      property.value.approval_state_id = response.data.approval_state_id;
    })
    .catch((error: any) => {
      console.error("Error al obtener datos:", error);
    });
...
function saveProperty() {
  ...
  backendServices
    .updateProperty(property_id.value, formData)
    .then((response: any) => {
      toast.add({
        title: "Propiedad actualizada con exito",
        color: "green",
        background: "dark:bg-slate-900",
        timeout: 3000,
      });
      router.push("/my-properties/property-list")
...
const getImages = async (imageUrl: string) => {
  ...
  const response = await backendServices.getImage(imageUrl);
        const file = new File([response.data], fileNameWithExtension, {type: `image/${fileExtension}` });
        return file; 
};
...
function changeStatus(status: number) {
  try{
    backendServices
    .changeStatusProperty(property_id.value, status)
    .then((response: any) => {
          if(response.status == 201){
            toast.add({
            title: "Propiedad Aprobada con exito",
            color: "green",
            background: "dark:bg-slate-900",
            timeout: 3000,
          });
        ...
        toast.add({
            title: "Propiedad Rechazada con exito",
            color: "green",
            background: "dark:bg-slate-900",
            timeout: 3000,
          });
